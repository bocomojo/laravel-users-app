...
class LiquidationImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            DB::transaction(function () use ($row, $sdoName, $cashAdvance, $preAuditorNames) {
                // Create liquidation
    ...
    : ($row['or_date'] ?? null),
                ...
                if (!empty($preAuditorNames)) {
                    $auditorIds = PreAuditor::whereIn(DB::raw('LOWER(name)'), collect($preAuditorNames)->map(fn($n) => strtolower($n))->toArray())
                        ->pluck('id');
